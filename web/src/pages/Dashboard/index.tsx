...
const Dashboard = () => {
  const timeElapsed = Date.now();
    ...
    const [selectedData1, setSelectedData1] = useState(`${dateNow.getFullYear()}-01-01T00:00`);
    const [selectedData2, setSelectedData2] = useState(dateNow.toISOString().slice(0,16));
  ...
  const handleClose = () => {
    ...
    const handleDataChange1 = useCallback((event: ChangeEvent<HTMLDataElement>) =>{
      const data1 = event.target.value;
      setSelectedData1(data1);
     },[]);

     const handleDataChange2 = useCallback((event: ChangeEvent<HTMLDataElement>) =>{
    ...
    useEffect(() =>{
      ...
      api.get('/pedidos/data/'+user.id+'/'+selectedData1+'/'+selectedData2)
      .then(response => {
    ...
    async function listDados(){
      api.get('/pedidosDados/'+user.id+'/'+selectedData1+'/'+selectedData2)
      .then(response => {
    ...
    async function listDadosGrafico(){
      api.get('/pedidosGrafico/'+user.id+'/'+selectedData1+'/'+selectedData2)
      .then(response => {
          ...
          <MenuSideTop />
            ...
            >
              ...
              <Card className= 'card-content' id='card1'>
                <CardContent>
               <h5><MonetizationOnIcon/> Total</h5> <h6 className='dados'>R$ {dados.map((dados:any, index) => <b key={index}>{dados.total}</b>)} </h6>
                </CardContent>
              </Card>
              <Card className= 'card-content' id='card2'>
                <CardContent>
                <h5><PaymentIcon/> A receber </h5> <h6 className='dados'>R$ {dados.map((dados:any, index) => <b key={index}>{dados.receber}</b>)}</h6>
                </CardContent>
              </Card>
              <Card className='card-content' id='card3'>
                <CardContent >
                <h5><MonetizationOnIcon/> Lucro Final</h5> <h6 className='dados'>R$ {dados.map((dados:any, index) => <b key={index}>{dados.lucro}</b>)}</h6>
                </CardContent>
              </Card>
              <Card className='card-content' id='card4'>
                <CardContent >
                <h5><CallReceivedIcon/> Recebido</h5> <h6 className='dados'>R$ {dados.map((dados:any, index) => <b key={index}>{dados.rebecido}</b>)} </h6>
                </CardContent>
              </Card>
              ...
              <Button id="card6" style={{marginLeft:'3px', marginTop: '5px', border: 'solid 0.1px', maxWidth:'1%', minWidth:'2%'}} onClick={resetar}><CancelIcon color='secondary'/></Button>
              </div>
              <div id= 'grid4' className='grafico'>
                    <AreaChart style={{marginLeft: '-3%', marginTop: '2%' }} width={400} height={300} data={dataGrafico}
                    ...
                    <XAxis dataKey="dataTotal" label={{ value: 'Data',offset:-2, position:"insideBottom" }} />
                    <YAxis />
                    <CartesianGrid strokeDasharray="3 3" />
                    <Tooltip />
                    <Legend verticalAlign="top" height={36}/>
                        <Area type="monotone" dataKey="lucro" stroke="#00a65a" fillOpacity={1} fill="url(#colorUv)" />
                    </AreaChart>
              </div>
              <div id='grid5'>
              <MaterialTable style={{ flex : 1, width: '98%'}}
              title="PEDIDOS"
              columns={[
              ...
              actions={[
                {icon:()=><div><CloudDownloadIcon/></div>,
